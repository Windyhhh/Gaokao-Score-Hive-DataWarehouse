-- ============================================================
-- 高考招生数据仓库 - OLAP分析脚本演示版
-- 包含8类OLAP操作，每类2条HQL语句，共16条查询
...
-- 查询4.1：2017-2019年，天津和青海，本科一批和本科二批，理科
SELECT
    t.year year,
    l.province_name province,
    b.batch_name batch,
...
-- 查询4.2：2018-2019年，广西，本科批次，文科和理科对比
SELECT
    t.year year,
    b.batch_name batch,
    c.candidate_type candidate_type,
...
-- 查询5.1：2019年平均录取分最高的前10所高校在三省的表现
SELECT
...
-- 查询5.2：天津市理科本科一批录取分最高的前15所高校（2017-2019年平均）
SELECT
    u.university_name university,
    u.university_province province,
    COUNT(*) year_count,
...
-- 查询6.2：青海省本科二批录取分最低的12所高校（2016-2019年）
SELECT
    u.university_name university,
    u.university_province province,
    c.candidate_type candidate_type,
...
-- 查询8.2：文科vs理科录取分数的四分位对比（2017-2019年）
SELECT
    c.candidate_type candidate_type,
    COUNT(*) total_records,
    ROUND(MIN(f.avg_score), 2) min_value,
